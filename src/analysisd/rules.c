...
RuleInfo *OS_CheckIfRuleMatch(struct _Eventinfo *lf, EventList *last_events,
    ...
    if (rule->program_name) {
        ...
        if (w_expression_match(rule->program_name, lf->program_name, NULL, rule_match, &lf->regex_time, &lf->regex_count)
    ...
    if (rule->id) {
        ...
        if (w_expression_match(rule->id, lf->id, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->id->negate) {
            return (NULL);
        }
    }
    ...
    if (rule->system_name) {
        ...
        if (w_expression_match(rule->system_name, lf->systemname, NULL, rule_match, &lf->regex_time, &lf->regex_count)
    ...
    if (rule->protocol) {
        ...
        if (w_expression_match(rule->protocol, lf->protocol, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->protocol->negate) {
            return (NULL);
        }
    }
    ...
    if (rule->match) {
        if (w_expression_match(rule->match, lf->log, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->match->negate) {
            return (NULL);
        }
    }
    ...
    if (rule->regex) {
        bool matches = w_expression_match(rule->regex, lf->log, NULL, rule_match, &lf->regex_time, &lf->regex_count);
    ...
    if (rule->action) {
        ...
        if (w_expression_match(rule->action, lf->action, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->action->negate) {
            return (NULL);
        }
    }
    ...
    if (rule->url) {
        ...
        if (w_expression_match(rule->url, lf->url, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->url->negate) {
            return (NULL);
        }
    }
    ...
    if (rule->location) {
        ...
        if (w_expression_match(rule->location, lf->location, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->location->negate) {
            return (NULL);
        }
    }
    ...
    for (i = 0; i < Config.decoder_order_size && rule->fields[i]; i++) {
        ...
        bool matches = w_expression_match(rule->fields[i]->regex, field, NULL, rule_match, &lf->regex_time, &lf->regex_count);
    ...
    if (rule->alert_opts & DO_PACKETINFO) {
        ...
        if (rule->srcip) {
            ...
            if (w_expression_match(rule->srcip, lf->srcip, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->srcip->negate) {
                return (NULL);
            }
        }
        ...
        if (rule->dstip) {
            ...
            if (w_expression_match(rule->dstip, lf->dstip, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->dstip->negate) {
                return (NULL);
            }
        }
        ...
        if (rule->srcport) {
            ...
            if (w_expression_match(rule->srcport, lf->srcport, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->srcport->negate) {
                return (NULL);
            }
        }
        if (rule->dstport) {
            ...
            if (w_expression_match(rule->dstport, lf->dstport, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->dstport->negate) {
                return (NULL);
            }
        }
    } /* END PACKET_INFO */
    ...
    if (rule->alert_opts & DO_EXTRAINFO) {
        ...
        if (rule->user) {
            if (lf->dstuser) {
                if (w_expression_match(rule->user, lf->dstuser, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->user->negate) {
                    return (NULL);
                }
            } else if (lf->srcuser) {
                if (w_expression_match(rule->user, lf->srcuser, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->user->negate) {
                    return (NULL);
                }
            } else {
                /* no user set */
        ...
        if(rule->srcgeoip) {
            ...
            if (w_expression_match(rule->srcgeoip, lf->srcgeoip, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->srcgeoip->negate) {
                return NULL;
            }
        }
        ...
        if(rule->dstgeoip) {
            ...
            if (w_expression_match(rule->dstgeoip, lf->dstgeoip, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->dstgeoip->negate) {
                return NULL;
            }
        }
        ...
        if (rule->data) {
            ...
            if (w_expression_match(rule->data, lf->data, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->data->negate) {
                return (NULL);
            }
        }
        ...
        if (rule->extra_data) {
            ...
            if (w_expression_match(rule->extra_data, lf->extra_data, NULL, rule_match, &lf->regex_time, &lf->regex_count)
        ...
        if (rule->hostname) {
            ...
            if (w_expression_match(rule->hostname, lf->hostname, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->hostname->negate) {
                return (NULL);
            }
        }
        ...
        if (rule->status) {
            ...
            if (w_expression_match(rule->status, lf->status, NULL, rule_match, &lf->regex_time, &lf->regex_count) == rule->status->negate) {
                return (NULL);
            }
        }
